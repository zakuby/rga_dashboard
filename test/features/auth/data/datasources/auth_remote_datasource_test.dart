...
void main() {
  ...
  group('AuthRemoteDataSourceImpl', () {
    group('login', () {
      test(
        'should return UserModel with valid credentials',
        () async {
      ...
      test(
        'should extract name from email correctly',
        () async {
          final result = await dataSource.login(
            email: 'test@example.com',
            password: 'password123',
      ...
      test(
        'should throw AuthenticationException for invalid email',
        () async {
          expect(
            () => dataSource.login(
      ...
      test(
        'should throw AuthenticationException for both invalid',
        () async {
          expect(
            () => dataSource.login(
              email: 'wrong@example.com',
      ...
      test(
        'should set lastLoginAt to current time',
        () async {
          ...
          expect(
            result.lastLoginAt.isBefore(after.add(const Duration(seconds: 1))),
            isTrue,
          );
        },
        timeout: const Timeout(Duration(seconds: 10)),
